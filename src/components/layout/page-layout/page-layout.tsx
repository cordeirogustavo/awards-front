...
const HeaderContent: React.FC<{ children: React.ReactNode }> = ({ children }) => {
...
const Actions: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return <div className="my-4 flex w-full flex-row justify-end gap-2 sm:my-0">{children}</div>
}

const Content: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return <div className="flex min-h-0 w-full flex-1 flex-col gap-4">{children}</div>
}
